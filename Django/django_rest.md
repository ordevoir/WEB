...
>Для конфигурации, использующей Python 3.6 и MySQL, можно сразу установить все необходимые библиотеки (включая библиотеки аутентификации) для Django REST framework (Representational State Transfer):
>
>    ```
...
Несмотря на то, что обмен данными между клиентом и сервером осуществляется посредством AJAX, для веб приложений необходимо отправить клиенту HTML документ, содержащий клиентскую часть приложения. Если мы имем дело с веб-приложением, то HTML документ представляет собой не просто статическую страницу, а содержит множество интерактивных элементов. В частности, это может быть одностраничное приложение (SPA), созданное на фреймворках Vue, React или Angular и тд.

Пусть имеется index.html – клиентская часть веб-приложения. Для того, чтобы при запуске страницы клиент получил этот документ, можно создать контроллер, возвращающий HTML, аналогично тому, как это обычно делается в Django:
...
>Следует отметить, что index.html размещается не в директории самого приложения labor/, а в labor/templates/labor/ . Статические файлы размещаются отдельно при deploy.

На стадии разработки, когда проект Vue запускается через CLI (по локальному хосту 8080), можно просто запустить также сервер Django (по локальному хосту 8000) и производить туда непосредственно запросы.

...
Часто возникает необходимость динамически изменять CSS-классы и inline-стили элементов в зависимости от состояния приложения. 
Управление классами элементов осуществляется путем привязки переменной к атрибуту `class` директивой `v-bind`. Для управления inline-стилем следует привязываться к атрибуту `style`.


Vue предоставляет дополнительные возможности директивы v-bind для работы с class и style. Эти атрибуты кроме строковых значений могут принимать массивы или объекты.
